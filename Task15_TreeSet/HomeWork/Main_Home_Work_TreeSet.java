...
Создадим класс-перечисление Group.java (enum):
WORK("Работа");
FRIENDS("Друзья");
FAMILY("Семья").
Для хранения контактов предлагаю использовать HashMap, в качестве ключа использовать номер телефона, а в качестве значения хранить контактные данные. Лучше вынести эту коллекцию в отдельный класс Contacts.java, где добавить методы для работы с этой коллекцией.
...
*
* */
public class Main_Home_Work_TreeSet {
